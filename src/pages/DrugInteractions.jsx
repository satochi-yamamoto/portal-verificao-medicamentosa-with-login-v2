...
const DrugInteractions = () => {
  ...
  const loadRealInteractions = async () => {
    try {
      ...
      console.log('✅ Interações carregadas:', realInteractions?.length || 0)
      
      // Carregar estatísticas
      const interactionStats = await interactionService.getInteractionStats()
      console.log('✅ Estatísticas carregadas:', interactionStats)
      ...
      if (formattedInteractions.length === 0) {
        setError('Nenhuma interação encontrada. Continue consultando medicamentos para popular esta base.')
      }
      
    } catch (err) {
      console.error('❌ Erro ao carregar interações:', err)
      setError(`Erro ao carregar dados de interações: ${err.message}`)
